...
function NewArrivalsContent() {
  ...
  const [selectedSection, setSelectedSection] = useState<CollectionSection | 'all'>('new');
  ...
  // Filtered Products Memo (New Arrivals Focus)
  const filteredProducts = useMemo(() => {
    ...
    // 2. Collection Section Filter (Defaults to 'new' or newest arrivals)
    if (selectedSection !== 'all') {
      result = result.filter(p => {
        if (p.collectionSections?.includes(selectedSection)) return true;
        const titleLower = p.title.toLowerCase();
        if (selectedSection === 'new') {
          return p.isNewArrival || titleLower.includes('new') || titleLower.includes('2026') || titleLower.includes('latest');
        }
        return false;
      });
    } else {
      result = result.filter(p => {
        return p.isNewArrival || p.collectionSections?.includes('new') || p.title.toLowerCase().includes('new');
      });
    }
  ...
  return (
    <div style={{ backgroundColor: '#faf7f0', color: '#1a1209', minHeight: '100vh', fontFamily: "'Jost', sans-serif" }}>
        ...
        <div className="new-hero-grid">
          ...
          <div style={{ textAlign: 'left' }}>
            ...
            <p style={{ fontFamily: "'Jost', sans-serif", fontSize: 'clamp(13px, 1.1vw, 15px)', lineHeight: 1.6, color: 'rgba(255,255,255,0.82)', maxWidth: '480px', margin: '0 0 22px', fontWeight: 300 }}>
              Be the first to discover our newest horological additions. Featuring refined cases, scratch-resistant sapphire crystal glass, and sleek modern dials.
            </p>
      ...
      <main id="new-catalog" style={{ maxWidth: '1400px', margin: '0 auto', padding: '60px 24px 100px' }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            ...
            />
            <p style={{ color: 'rgba(26,18,9,0.5)', fontSize: '13px' }}>Loading New Arrival timepieces…</p>
          </div>
        ) : error ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 20px', background: '#ffffff', borderRadius: '16px', border: '1px dashed rgba(26,18,9,0.15)' }}>
            <p style={{ fontSize: '15px', color: 'rgba(26,18,9,0.5)', margin: 0 }}>No new arrival timepieces matched your search criteria.</p>
            <button
