...
interface SportsHeroTypewriterProps {
  title?: string;
  subtitle?: string;
}

export default function SportsHeroTypewriter({
  title = 'Sport Timepiece Collection',
  subtitle = 'Engineered for those who never settle. Discover bold, high-performance timepieces crafted with Japanese precision movements and Dubai-certified quality.',
}: SportsHeroTypewriterProps) {
  ...
  useEffect(() => {
    ...
    const runLoop = async () => {
      while (!isCancelled) {
        ...
        // 2. Type Title ("Sport Timepiece Collection")
    ...
    <>
      <style>{`
        ...
        .sports-caret {
          ...
          border-radius: 1px;
          animation: sports-caret-pulse 0.85s cubic-bezier(0.4, 0, 0.2, 1) infinite;
        }
        .sports-caret-sub {
          width: 1.8px;
          height: 0.88em;
      ...
      <div className="sports-title-container" style={{ position: 'relative', marginBottom: '14px', width: '100%' }}>
        ...
        <h1 className="sports-title-live" aria-hidden="true">
          <span>
            {typedTitle}
            {cursorPos === 'title' && <span className="sports-caret" />}
          </span>
        </h1>
      </div>
      ...
      <div className="sports-sub-container" style={{ position: 'relative', maxWidth: '480px', marginBottom: '22px', width: '100%' }}>
        ...
        <p className="sports-subtitle-live" aria-hidden="true">
          <span>
            {typedSubtitle}
            {cursorPos === 'sub' && <span className="sports-caret sports-caret-sub" />}
          </span>
        </p>
      </div>
    </>
