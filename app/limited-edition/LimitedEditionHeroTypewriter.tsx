...
interface LimitedEditionHeroTypewriterProps {
  title?: string;
  subtitle?: string;
}

export default function LimitedEditionHeroTypewriter({
  title = 'Limited Edition Reserve',
  subtitle = 'Strictly numbered production runs. Mastercrafted with premium materials, automatic & precision movements, and bespoke luxury packaging.',
}: LimitedEditionHeroTypewriterProps) {
  ...
  useEffect(() => {
    ...
    const runLoop = async () => {
      while (!isCancelled) {
        ...
        // 2. Type Title ("Limited Edition Reserve")
    ...
    <>
      <style>{`
        ...
        .limited-caret {
          ...
          border-radius: 1px;
          animation: limited-caret-pulse 0.85s cubic-bezier(0.4, 0, 0.2, 1) infinite;
        }
        .limited-caret-sub {
          width: 1.8px;
          height: 0.88em;
      ...
      <div className="limited-title-container" style={{ position: 'relative', marginBottom: '14px', width: '100%' }}>
        ...
        <h1 className="limited-title-live" aria-hidden="true">
          <span>
            {typedTitle}
            {cursorPos === 'title' && <span className="limited-caret" />}
          </span>
        </h1>
      </div>
      ...
      <div className="limited-sub-container" style={{ position: 'relative', maxWidth: '480px', marginBottom: '22px', width: '100%' }}>
        ...
        <p className="limited-subtitle-live" aria-hidden="true">
          <span>
            {typedSubtitle}
            {cursorPos === 'sub' && <span className="limited-caret limited-caret-sub" />}
          </span>
        </p>
      </div>
    </>
