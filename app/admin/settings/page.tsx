...
export default function SettingsPage() {
  ...
  const fetchProfileAndData = async () => {
    try {
      ...
      // If user is Admin, load all staff accounts
  ...
  const handleSelfUpdate = async (e: FormEvent) => {
    ...
    try {
      const targetId = currentUser?.role === 'staff' ? (currentUser as any).adminId : null;
      if (!targetId) throw new Error('Missing adminId credentials');
  ...
  return (
    <PermissionGate permission="settings_manage">
      <div className="space-y-6 font-['Jost'] text-[#1a1209]">
        ...
        <div>
          ...
          <p className="text-[#1a1209]/60 mt-1">
            {currentUser?.role === 'admin' 
              ? 'Manage administrator parameters, credentials, and staff operational profiles.' 
              : 'View your staff profile, check account expiration limits, and edit credentials.'}
          </p>
        </div>

        {/* ── ADMIN VIEW: FULL STAFF CRUD MANAGEMENT ── */}
        {currentUser?.role === 'admin' && (
        ...
        {/* ── STAFF VIEW: countdown & profile credential changes ── */}
        {currentUser?.role === 'staff' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            
            {/* Left Card: Staff Account Meta */}
            <div className="bg-white border border-[#1a1209]/10 rounded-xl p-6 shadow-sm space-y-6">
              <h3 className="font-semibold text-lg border-b border-[#1a1209]/5 pb-3">Staff Profile Details</h3>
            ...
            {/* Right Card: Credential Modification Form (Permanent Staff Only) */}
                  ...
                  {editIsTemporary && (
                    <div>
                      ...
                      <input
                        ...
                        className="w-full accent-[#8B6914]"
                      />
                      <span className="text-[10px] text-[#1a1209]/40">Slider range: 1 to 30 days.</span>
                    </div>
                  )}

                  <div>
                    <label className="block text-[11px] font-semibold tracking-wider uppercase text-[#1a1209]/70 mb-2">Account status</label>
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 text-sm">
                        <input
                          type="radio"
