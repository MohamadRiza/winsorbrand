...
export default function AddProductPage() {
  ...
  // ✅ Properly typed formData with non-nullable defaults
  const [formData, setFormData] = useState({
    ...
    thumbnail: EMPTY_ASSET,  // ✅ Non-nullable default
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate color variants have required fields
    for (const variant of formData.colorVariants) {
      if (!variant.colorName?.trim() || !variant.image?.url) {
        toast.error('Please complete all color variant fields');
        return;
      }
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="font-['Jost'] text-[#1a1209]/60 mt-1">
            Create a new luxury timepiece listing
          </p>
        </div>
        <button
          type="button"
