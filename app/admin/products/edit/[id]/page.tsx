...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Populate targetGender
      const specs = product.specifications || {};
      let initialGender: 'Gents' | 'Ladies' | 'Unisex' = 'Gents';
      for (const k of Object.keys(specs)) {
        if (k.toLowerCase() === 'gender') {
          const val = specs[k].toLowerCase();
          if (val.includes('lady') || val.includes('women') || val.includes('ladies')) initialGender = 'Ladies';
          else if (val.includes('unisex')) initialGender = 'Unisex';
          else initialGender = 'Gents';
          break;
        }
      }
      setTargetGender(initialGender);
    ...
    try {
      ...
      if (tasks.length > 0) {
        ...
        for (let i = 0; i < tasks.length; i++) {
          ...
          setUploadProgress(prev => ({
            ...
            statusMessage: remainingTasks > 0 
              ...
              : 'Cloud assets optimized. Updating database listing...',
        ...
        setUploadProgress(prev => ({
          ...
          overallPercent: 94,
          stage: 'saving',
          statusMessage: 'Saving updated product details to database...',
          estimatedSecondsRemaining: 1,
        }));
      } else {
        // Fast modal for database update when only text fields changed
        setUploadProgress({
          isOpen: true,
          overallPercent: 88,
          currentFileName: '',
          ...
          totalBytes: 0,
          stage: 'saving',
          statusMessage: 'Updating product details in database...',
          estimatedSecondsRemaining: 1,
          uploadSpeedBytesPerSec: 0,
        });
      }

      // 3. Submit updated payload to API
